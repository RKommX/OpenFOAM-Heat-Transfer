...
5) run command : "paraFoam" or "reconstructPar" // to open paraFoam for viewing the results
 
Next run the parafoam and see the contours and other results.

(Note: Some Allrun files may not work with every OpenFOAM version. If you encounter issues, delete those folders and follow the commands above
or

1) run command: ./Allrun //to run all the commands automatically

After running the above command the simulation will start to run, after completion type 
2) run command : "paraFoam" or "paraFoam -builtin" to visualize and post-process
